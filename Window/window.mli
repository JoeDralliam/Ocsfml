...
module Event :
  sig
    ...
    (** an alias to Mouse.button for internal purpose ; you should use Mouse.button in your programs *)
    type mouseButton =
	MouseLeft 
      | MouseRight
      | MouseMiddle
...
object
  ...
  (** deallocates the memory on c++ side ; don't forget to call this method at the end of your function to prevent memory leaks*)
  method destroy : unit -> unit
    
  (** for internal purpose only ; you should not use this method *)
...
sig
  ...
  external get_relative_position : window -> int * int = "Mouse_GetRelativePosition__impl"
  external set_position : int * int -> unit = "Mouse_SetPosition__impl"
  external set_relative_position : int * int -> window -> unit = "Mouse_SetRelativePosition__impl"
end
